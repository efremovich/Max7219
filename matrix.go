...
func (this *Matrix) SlideMessage(text string, font Font,
	...
	b := convertUnicodeToAscii(text, font.GetCodePage())
	buf := preparePatterns(b, font.GetLetterPatterns(), condensePattern)
...
func (this *Matrix) Blink() {
	for i := 0; i < 3; i++ {
		for x := 0; x < 14; x++ {
			time.Sleep(50 * time.Millisecond)
			this.Device.Brightness(byte(x))
		}
	}
	this.Device.Brightness(byte(7))
}
